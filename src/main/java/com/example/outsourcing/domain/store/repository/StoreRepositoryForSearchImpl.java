...
public class StoreRepositoryForSearchImpl implements StoreRepositoryForSearch{
    ...
    @Override
    public List<StoreResponseDto> findStoresByArea(Point area, int size, OrderBy orderBy) {
    ...
    @Override
    public List<StoreResponseDto> findStoresByCategory(Point area, Long categoryId, int size, OrderBy orderBy) {
        String base = "SELECT new com.example.outsourcing.domain.store.dto.response.StoreResponseDto("
                + "s, "
                + "CAST( ST_Distance_Sphere(s.location, :area) AS double), "
                + "CAST( COALESCE(AVG(r.rate), 0) AS double)) "
    ...
    @Override
    public List<StoreResponseDto> findStoresBySearch(Point area, String search, int size, OrderBy orderBy) {
        String base = "SELECT new com.example.outsourcing.domain.store.dto.response.StoreResponseDto("
                + "s, "
                + "CAST( ST_Distance_Sphere(s.location, :area) AS double), "
                + "CAST( COALESCE(AVG(r.rate), 0) AS double)) "
