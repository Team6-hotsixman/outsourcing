...
public class ReviewController {
    ...
    public ResponseEntity<ReviewResponse> saveReview(@Auth AuthUser authUser,
                                                     @PathVariable long orderId,
                                                     @RequestPart(value = "create", required = false) ReviewCreateRequest reviewCreateRequest,
                                                     @RequestPart(value = "images", required = false) List<MultipartFile> images) {

        ReviewResponse reviewResponse = reviewService.saveReview(orderId, authUser, reviewCreateRequest, images);
        return ResponseEntity.ok(reviewResponse);
    }

    @GetMapping("/reviews/stores/{storeId}")
    public ResponseEntity<List<ReviewResponse>> getReviews(@PathVariable long storeId,
        ...
        List<ReviewResponse> reviews = reviewService.getReviews(storeId, start, end, lastDate);

        return ResponseEntity.ok(reviews);
    }

    @PatchMapping("/reviews/{reviewId}")
    public ResponseEntity<ReviewResponse> updateReview(@Auth AuthUser authUser,
    ...
    public ResponseEntity<Void> deleteReview(@Auth AuthUser authUser,
                                             @PathVariable long reviewId) {
